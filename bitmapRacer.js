class Track {
  constructor() {
    ...
    if (!this.reverse) {
      this.startX = this.flipX
        ? (p.track.x - p.track.startX) / this.trackPPM
        : p.track.startX / this.trackPPM;
      this.startY = this.flipY
        ? (p.track.x - p.track.startY) / this.trackPPM
        : p.track.startY / this.trackPPM;
    } else {
      this.startX = this.flipX
        ? (p.track.x - p.track.startXRev) / this.trackPPM
        : p.track.startXRev / this.trackPPM;
      this.startY = this.flipY
        ? (p.track.x - p.track.startYRev) / this.trackPPM
        : p.track.startYRev / this.trackPPM;
    }
    // this.startX=220;
    // this.startY=220;
    // log(p.ctrack)
    ...
    for (let i = 0; i < Xi; i++) {
      ...
      for (let j = 0; j < Yi; j++) {
        ...
        if (lx == 0) {
          ...
          //colourless, not black
          sfcType = "tarmac";
        } else if ((hx > 150) & (hx <= 200) & (sx > 10) & (lx > 80)) {
          sfcType = "ice";
        } else if ((hx >= 70) & (hx <= 150)) {
          // green
          sfcType = "grass";
        } else if (((hx > 330) | (hx < 50)) & (lx < 75) & (lx > 10)) {
          //dark red/yellow i.e. brown
          sfcType = "mud";
        } else {
          sfcType = "unknown";
        }
  ...
  getImageData() {
    ...
    this.ctxScl.translate(
      0 + (this.Xi * this.trackScl) / 2,
      0 + (this.Yi * this.trackScl) / 2
    );
    ...
    this.ctxScl.drawImage(
      this.img,
      0,
      0,
      this.Xi * this.trackScl,
      this.Yi * this.trackScl
    );
  }
  drawGates(ctx, xc, yc) {
    ...
    ctx.moveTo(
      gate.left.x * this.trackScl + xc,
      gate.left.y * this.trackScl + yc
    );
    ctx.lineTo(
      gate.right.x * this.trackScl + xc,
      gate.right.y * this.trackScl + yc
    );
    ctx.stroke();
    ctx.beginPath();
    // ctx.fillStyle = 'blue';
...
class Car {
  constructor() {
    ...
    this.momI =
      p.car.momIfac * this.m * ((this.l / 2) ** 2 + (this.w / 2) ** 2); // moment of inertia
    // log("momI", this.momI)
    ...
    this.coordMatHUDbody = this.relCoords.body.map((x) => [
      x[0] * this.w,
      x[1] * this.l - this.rearLength,
    ]);
    //scaled for drawing on track
    this.coordMatBody = this.coordMatHUDbody.map((x) => x.map((x) => x * PPM));
    // cockpit
    this.coordMatCockpit = this.relCoords.cockpit.map((x) => [
      PPM * x[0] * this.w,
      PPM * (x[1] * this.l - this.rearLength),
    ]);
    //axels
    this.coordMatAxels = this.relCoords.axels.map((x) => [
      PPM * x[0] * this.w,
      PPM * (x[1] * this.l - this.rearLength),
    ]);
    ...
    this.brakeWheelHUD =
      this.brakeFade <= 0.5 ? this.wheels[0] : this.wheels[2];
    this.maxTorqueHUD = Math.max(
      (this.fade * this.torqueMax) / 2,
      ((1 - this.fade) * this.torqueMax) / 2
    );
    this.maxBrakeHUD = Math.max(
      (this.brakeFade * this.brakeMax) / 2,
      ((1 - this.fade) * this.brakeMax) / 2
    );
  ...
  readTrack() {
    this.wheels.forEach(function (wheel) {
      ...
      [wheel.sfc_drag, wheel.sfc_mu, wheel.sfc_void] = track.get_sfc_params(
        wheel.xa,
        wheel.ya
      );
    });
  ...
  draw(ctx, xc, yc) {
    ...
    this.wheels.forEach((wheel) =>
      wheel.draw(ctx, this.rotMat, this.x, this.y, xc, yc)
    );
  ...
  drawHUD(ctx) {
    ...
    ctx.stroke();
    this.wheels.forEach((wheel) =>
      wheel.drawHUD(ctx, this, HUDx, HUDy, HUDscl, HUDforceScl)
    );
  ...
  control(inputState) {
    if (inputState.left) {
      this.wheels[0].theta = Math.min(
        this.steeringMax,
        this.wheels[0].theta + this.steeringRate * dt
      );
      this.wheels[0].rotMat = fs.calcRotMat(this.wheels[0].theta);
      this.wheels[1].theta = Math.min(
        this.steeringMax,
        this.wheels[1].theta + this.steeringRate * dt
      );
      this.wheels[1].rotMat = fs.calcRotMat(this.wheels[1].theta);
    } else if (inputState.right) {
      this.wheels[0].theta = Math.max(
        -this.steeringMax,
        this.wheels[0].theta - this.steeringRate * dt
      );
      this.wheels[0].rotMat = fs.calcRotMat(this.wheels[0].theta);
    ...
    if (inputState.up) {
      // front wheel accel
      this.wheels[0].torque = Math.min(
        (this.torqueMax / 2) * (1 - this.fade),
        this.wheels[0].torque + (this.torqueRate / 2) * dt
      );
      this.wheels[1].torque = Math.min(
        (this.torqueMax / 2) * (1 - this.fade),
        this.wheels[1].torque + (this.torqueRate / 2) * dt
      );
      this.wheels[2].torque = Math.min(
        (this.torqueMax / 2) * this.fade,
        this.wheels[2].torque + (this.torqueRate / 2) * dt
      );
      this.wheels[3].torque = Math.min(
        (this.torqueMax / 2) * this.fade,
        this.wheels[3].torque + (this.torqueRate / 2) * dt
      );
    } else if (!inputState.down) {
      this.wheels[0].torque = Math.max(
        0,
        this.wheels[0].torque - (this.torqueRate / 2) * dt
      );
      this.wheels[1].torque = Math.max(
        0,
        this.wheels[1].torque - (this.torqueRate / 2) * dt
      );
      this.wheels[2].torque = Math.max(
        0,
        this.wheels[2].torque - (this.torqueRate / 2) * dt
      );
      this.wheels[3].torque = Math.max(
        0,
        this.wheels[3].torque - (this.torqueRate / 2) * dt
      );
    }
    if (inputState.down) {
      ...
      if (this.ulon > 0.1) {
        ...
        // 4 wheel braking
        this.wheels[0].brake = Math.min(
          (this.brakeMax / 2) * (1 - this.brakeFade),
          this.wheels[0].brake + (this.brakeRate / 2) * dt
        );
        this.wheels[1].brake = Math.min(
          (this.brakeMax / 2) * (1 - this.brakeFade),
          this.wheels[1].brake + (this.brakeRate / 2) * dt
        );
        this.wheels[2].brake = Math.min(
          (this.brakeMax / 2) * this.brakeFade,
          this.wheels[2].brake + (this.brakeRate / 2) * dt
        );
        this.wheels[3].brake = Math.min(
          (this.brakeMax / 2) * this.brakeFade,
          this.wheels[3].brake + (this.brakeRate / 2) * dt
        );
      }
      if (this.ulon <= 0.1) {
        this.wheels[0].brake = Math.max(
          0,
          this.wheels[0].brake - (this.brakeRate / 2) * dt
        );
        this.wheels[1].brake = Math.max(
          0,
          this.wheels[1].brake - (this.brakeRate / 2) * dt
        );
        this.wheels[2].brake = Math.max(
          0,
          this.wheels[2].brake - (this.brakeRate / 2) * dt
        );
        this.wheels[3].brake = Math.max(
          0,
          this.wheels[3].brake - (this.brakeRate / 2) * dt
        );
  ...
  mechV3() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // max Force from wheel (excluding  sfc drag), occurs at and above alpha0 in cornering, limits thrust and brake
      wh.Fmax = this.mu * wh.sfc_mu * wh.load;
      ...
      //accelerating
      wh.n.Fthrust.lon =
        cosTh * Math.sign(wh.torque) * Math.min(wh.Fmax, Math.abs(wh.torque));
      wh.n.Fthrust.lat =
        sinTh * Math.sign(wh.torque) * Math.min(wh.Fmax, Math.abs(wh.torque));

      // braking
      let BRKthresh = 0.1;
      if (Math.abs(wh.n.u.lonWheel) < BRKthresh) {
        wh.n.Fbrake.lon =
          -Math.sign(wh.n.u.lonWheel) *
          Math.min(wh.Fmax, (wh.n.u.lonWheel / BRKthresh) * wh.brake) *
          cosTh;
        wh.n.Fbrake.lat =
          -Math.sign(wh.n.u.lonWheel) *
          Math.min(wh.Fmax, (wh.n.u.lonWheel / BRKthresh) * wh.brake) *
        ...
        wh.n.Fbrake.lon =
          -Math.sign(wh.n.u.lonWheel) * Math.min(wh.Fmax, wh.brake) * cosTh;
        wh.n.Fbrake.lat =
          -Math.sign(wh.n.u.lonWheel) * Math.min(wh.Fmax, wh.brake) * sinTh;
      }
      ...
      if (Math.abs(wh.n.u.lonWheel) < RRthresh) {
        wh.n.Frollres.lon =
          ...
          Math.min(
            wh.Fmax,
            Math.abs((wh.n.u.lonWheel / RRthresh) * this.rollK)
          ) *
          cosTh;
        wh.n.Frollres.lat =
          -Math.sign(wh.n.u.lonWheel) *
        ...
        wh.n.Frollres.lon =
          -Math.sign(wh.n.u.lonWheel) * Math.min(wh.Fmax, this.rollK) * cosTh;
        wh.n.Frollres.lat =
          -Math.sign(wh.n.u.lonWheel) * Math.min(wh.Fmax, this.rollK) * sinTh;
      }
      ...
      // slip angle in degs
      wh.n.Fcorn.alpha =
        (180 / Math.PI) * Math.atan(wh.n.u.latWheel / wh.n.u.lonWheel);
      // }
      // else{
      //     wh.n.Fcorn.alpha = 0
      // }

      let cs = wh.Fmax / alpha0; //cornering stiffness in N/deg. Varies with load.

      if (this.U < 0.1) {
        wh.skidFac = 0;
        wh.n.Fcorn.lat = -wh.n.u.latWheel * cosTh * cs * 0.1;
        wh.n.Fcorn.lon = wh.n.u.latWheel * sinTh * cs * 0.1;
        // log('corn: slow', this.stiffness, wh.n.Fcorn.lon)
      } else if (Math.abs(wh.n.Fcorn.alpha) < alpha0) {
        wh.n.Fcorn.lat =
          -Math.sign(wh.n.u.lonWheel) * wh.n.Fcorn.alpha * cs * cosTh;
        wh.n.Fcorn.lon =
          Math.sign(wh.n.u.lonWheel) * wh.n.Fcorn.alpha * cs * sinTh;
        wh.skidFac = 1;
      ...
      // wh.n.Fcorn.lon = 0;
      // wh.n.Fcorn.lat = this.m * this.ulon**2  / (this.l / Math.sin(this.wheels[0].theta))/4;
      // wh.skidFac=0

      // calc wheel resultants, add to car total resultants
      wh.n.Fres.lon =
        wh.n.Fthrust.lon +
        wh.n.Fbrake.lon +
        wh.n.Frollres.lon +
    ...
    this.Fxy = fs.matrixProd(
      [[this.n.Fres.lat, this.n.Fres.lon]],
      fs.calcRotMat(this.theta)
    )[0];
    ...
    this.loadRight =
      (this.m / this.w) * ((this.g * this.w) / 2 + this.height * this.alat);
    this.loadLeft = this.W - this.loadRight;

    this.downForceFront =
      ((this.airDrag * this.CL) / 4) * (1 - this.fadeDownforce);
    this.downForceRear = ((this.airDrag * this.CL) / 4) * this.fadeDownforce;

    // log("downforceRear", this.downForceRear)
    this.loadFrontRight =
      (this.loadFront * this.loadRight) / this.W + this.downForceFront;
    this.loadFrontLeft =
      (this.loadFront * this.loadLeft) / this.W + this.downForceFront;
    this.loadRearRight =
      (this.loadRear * this.loadRight) / this.W + this.downForceRear;
    this.loadRearLeft =
      (this.loadRear * this.loadLeft) / this.W + this.downForceRear;
    ...
    // log(this.loadRear);
    this.steeringMax =
      (this.steeringMaxBase * +this.steeringUscl) /
      (this.steeringUscl + this.U);
    ...
    this.U = (this.ux ** 2 + this.uy ** 2) ** 0.5;
    ...
    if (this.headOff > Math.PI) {
      this.headOff = this.headOff - 2 * Math.PI;
    }
    if (this.headOff < -Math.PI) {
      this.headOff = this.headOff + 2 * Math.PI;
    }
  ...
  mechV2() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // braking
      if (Math.abs(wh.n.u.lonWheel) < 0.1) {
        wh.n.Fbrake.lon = (-wh.n.u.lon / 0.1) * wh.brake * wh.sfc_mu * cosTh;
        wh.n.Fbrake.lat = (-wh.n.u.lon / 0.1) * wh.brake * wh.sfc_mu * sinTh;
      } else {
        wh.n.Fbrake.lon =
          -Math.sign(wh.n.u.lonWheel) * wh.brake * wh.sfc_mu * cosTh;
        wh.n.Fbrake.lat =
          -Math.sign(wh.n.u.lonWheel) * wh.brake * wh.sfc_mu * sinTh;
      }
      // rolling resistance
      if (Math.abs(wh.n.u.lonWheel) < 0.1) {
        ...
        wh.n.Frollres.lon =
          -Math.sign(wh.n.u.lonWheel) * this.Crr * wh.sfc_mu * cosTh;
        wh.n.Frollres.lat =
          -Math.sign(wh.n.u.lonWheel) * this.Crr * wh.sfc_mu * sinTh;
      }
      //surface drag
      wh.n.Fdrag.lon = -wh.n.u.lonWheel * wh.sfc_drag * this.CD;
      wh.n.Fdrag.lat = -wh.n.u.latWheel * wh.sfc_drag * this.CD;
      ...
      if (this.ulon < 0.1) {
        wh.skidFac = 0;
        wh.n.Fcorn.lat = -wh.n.u.latWheel * cosTh * this.stiffness * 0.1;
        wh.n.Fcorn.lon = wh.n.u.latWheel * sinTh * this.stiffness * 0.1;
        // log('corn: slow', this.stiffness, wh.n.Fcorn.lon)
    ...
    this.loadFront =
      (this.m * (this.rearLength - this.height * this.alon)) / this.l;
    this.loadRear =
      (this.m * (this.frontLength + this.height * this.alon)) / this.l;
    this.loadFrontRight =
      (this.loadFront * (this.w / 2 + this.height * this.alat)) / this.w;
    this.loadFrontLeft =
      (this.loadFront * (this.w / 2 - this.height * this.alat)) / this.w;
    this.loadRearRight =
      (this.loadRear * (this.w / 2 + this.height * this.alat)) / this.w;
    this.loadRearLeft =
      (this.loadRear * (this.w / 2 - this.height * this.alat)) / this.w;
...
class TouchButton {
  ...
  draw(ctx) {
    ...
    ctx.fillText(
      this.txt,
      this.x0 * X + (this.w * X) / 2,
      this.y0 * Y + (this.h * Y) / 2
    );
  }
  contains(ex, ey) {
    return (
      (ex > this.x0 * X) &
      (ex < this.x0 * X + this.w * X) &
      (ey > this.y0 * Y) &
      (ey < this.y0 * Y + this.h * Y)
    );
  }
...
class LapCounter {
  ...
  draw(ctx) {
    ...
    if (this.void) {
      ...
      ctx.font = fontSizeBase * pixRat + "px " + this.fontFamily;
      ctx.fillText(
        "Lap " + this.lapn + "/" + multilapn,
        X / 2,
        Y - (Y / 3) * isTouch - 5 * pixRat
      );
    }
  }
  lapComplete() {
    this.completeLapTimePh = Math.round(
      ((n - (1 - this.bez) - this.n0) * 1000) / Fps.fps
    );
  ...
  resetStart() {
    // button reset, dont trigger ghost
    this.t0 = Date.now();
    this.n0 = n - (1 - this.bez);
    this.nextCheck = 0;
    this.void = false;
  ...
  checkGate(nGate, gate, oldPoint, newPoint) {
    ...
    this.intersection = fs.intersects(
      oldPoint.x,
      oldPoint.y,
      newPoint.x,
      newPoint.y,
...
class HiScores {
  constructor() {
    ...
    if (localStorage.getItem("versionTimes")) {
      ...
      let versionTimes = this.versionTimesList.filter((obj) => {
  ...
  draw(ctx) {
    ...
    for (let i = 0; i < this.n; i++) {
      ctx.fillText(
        (i + 1).toString() + " " + fs.formatDuration(this.times[i]),
        this.x,
        this.y + (i + 1.2) * this.dy
      );
    }
...
class Name {
  constructor() {
    ...
    document
      ...
      .addEventListener("click", fs.submitName, { passive: true });
    // log(this)
  }
  draw(ctx) {
    // let x0 = X - this.w;
    // let y0 = Y - (isTouch * Y) / 3 - this.h;
    // ctx.beginPath()
    // ctx.strokeStyle = "white";
  ...
  contains(ex, ey) {
    let x0 = X - this.w;
    let y0 = Y - (isTouch * Y) / 3 - this.h;
    return (ex > x0) & (ex < x0 + this.w) & (ey > y0) & (ey < y0 + this.h);
  }
  pointerDownHandler(ex, ey, en) {
    // log('PD')
    if (this.contains(ex, ey)) {
...
class ViewMode {
  ...
  draw(ctx) {
    // let x0 = X - this.w;
    // let y0 = Y - (isTouch * Y) / 3 - this.fontsize * 5 - this.h;
    // ctx.beginPath()
    // ctx.strokeStyle = "white";
    // ctx.rect(x0, y0, this.w, this.h)
    // ctx.stroke();
    ...
    ctx.fillText(
      "Cam: " + this.text,
      X - 5 * pixRat,
      Y - (isTouch * Y) / 3 - 5 * pixRat - this.fontsize * 5
    );
  }
  contains(ex, ey) {
    let x0 = X - this.w;
...
class Flash {
  ...
  draw(ctx) {
    ...
    if (
      (this.message == "") &
      (this.queue.length > 0) &
      (Date.now() - this.bTime > this.blankPeriod)
    ) {
...
class SessionLogger {
  constructor(timeTravelDaysURL, dev) {
    ...
    if (timeTravelDaysURL != 0) {
      // tt url arg overides session browser
      this.timeTravelDays = timeTravelDaysURL;
    } else {
      this.timeTravelDays = timeTravel.ttDays;
    }
    if (dayxmode) {
      //day selct mode in url arg
      ...
      this.currentTime =
        Date.now() / (1000 * 60 * 60 * 24) + this.timeTravelDays; //it offset for testing session changes
    ...
    let timeTillNext =
      (1 - (this.currentTime - this.currentSesh)) * 60 * 60 * 24;
  ...
  getLocalStats() {
    ...
    if (localStorage.getItem("versionTimes")) {
      ...
      let versionTimes = versionTimesList.filter((obj) => {
        return obj.version == this.version;
      });
      ...
      if (versionTimes.length > 0) {
        ...
        if (versionTimes[0].nLapsStat !== undefined) {
          // stats exist
          log("stats exist");
          return {
            laps: versionTimes[0].nLapsStat,
            voids: versionTimes[0].nVoidsStat,
            resets: versionTimes[0].nResetsStat,
          };
        }
        return stats0;
      } else {
  ...
  newLap(t) {
    this.currentnLaps++;
    if (t < this.currentBestLap) {
      this.currentBestLap = t;
    }
    this.qualTest();
  ...
  draw(ctx) {
    ...
    ctx.fillText(
      this.qText,
      X - 5 * pixRat,
      Y - (isTouch * Y) / 3 - 5 * pixRat - this.fontsize
    );
    if (dayxmode || timeTravel.ttDays != 0) {
      ctx.fillStyle = "dimGrey";
    }
    ctx.fillText(
      this.timeTillNextString + " remaining",
      X - 5 * pixRat,
      Y - (isTouch * Y) / 3 - 5 * pixRat - 3 * this.fontsize
    );
    // ctx.fillText("Streak: " + this.streak + " days", X - 5 * pixRat, Y - isTouch * Y / 3 - 5 * pixRat - 3 * this.fontsize)
    ctx.fillStyle = "white";
    if (this.currentRank) {
      ctx.fillText(
        "Rank: " + this.currentRank[0] + "/" + this.currentRank[1],
        X - 5 * pixRat,
        Y - (isTouch * Y) / 3 - 5 * pixRat - 4 * this.fontsize
      );
    }
    // if (this.yesterRank) {
    ...
    ctx.fillText(
      "Q-Streak: " + (this.inStreak + this.qualified),
      X - 5 * pixRat,
      Y - (isTouch * Y) / 3 - 5 * pixRat - 2 * this.fontsize
    );
...
class SessionSetter {
  ...
  randGen() {
    this.mult = this.randomElement([
      0.4, 0.6, 0.8, 0.95, 1.0, 1.1, 1.25, 1.5, 2.0,
    ]);
    // this.mult=2.0
  ...
  apply(p) {
    ...
    if (typeof this.colour === "string") {
      ...
      let colString =
        "rgb(" +
        this.colour[0].toString() +
        "," +
        this.colour[1].toString() +
        "," +
        this.colour[2].toString() +
        ")";
    ...
    p.trackSetup.metresPerPix =
      1 * this.scale.mpp * p.track.trackScale * carScale;
...
class Ghost {
  ...
  completeLap(t) {
    ...
    if (
      ...
      (timeTravel.ttDays == 0) &
      (t != 0) &
      ((t < this.savedLap.time) | (this.savedLap.time == 0))
    ) {
    ...
    if (
      ...
      (t != 0) &
      (name.name != null) &
      ((t < this.webLap.time) | (this.webLap.time == 0))
    ) {
  ...
  saveGhost(t) {
    ...
    if (!trackDev) {
      localStorage.setItem(
        "ghostVersion",
        JSON.stringify(sessionLogger.version)
      );
  ...
  draw(ctx, xc, yc) {
    ...
    if (this.ghostAvail & this.started & this.drawGhost) {
      if (this.n < this.savedLap.x.length) {
        ...
        x = fs.matrixTrans(x, [
          PPM * this.savedLap.x[this.n] + xc,
          PPM * this.savedLap.y[this.n] + yc,
        ]);
        ...
        this.wheels.forEach((wheel) =>
          wheel.draw(
            ctx,
            this.rotMat,
            this.savedLap.x[this.n],
            this.savedLap.y[this.n],
            xc,
            yc
          )
        );
  ...
  drawWeb(ctx, xc, yc) {
    ...
    if (this.webGhostAvail & this.started & this.drawWebGhost) {
      if (this.n < this.webLap.x.length) {
        ...
        let x = fs.matrixProd(car.coordMatAxels, this.rotMat);
        x = fs.matrixTrans(x, [
          PPM * this.webLap.x[this.n] + xc,
          PPM * this.webLap.y[this.n] + yc,
        ]);
        ...
        this.wheels.forEach((wheel) =>
          wheel.draw(
            ctx,
            this.rotMat,
            this.webLap.x[this.n],
            this.webLap.y[this.n],
            xc,
            yc
          )
        );
  ...
  loadFromWeb() {
    fetch(
      apiURL + "/get_ghost?version=" + sessionLogger.version + "&fps=" + Fps.fps
    )
      .then((response) => response.json())
      .then((data) => {
        // log(data)
  ...
  drawToggle() {
    // let cx0=0
    // let cy0=Y - isTouch * Y / 3 - 0 * pixRat - this.ch - this.fontsize;
    // ctx.beginPath()
    // ctx.strokeStyle = "white";
    // ctx.rect(cx0, cy0, this.cw, this.ch)
    // ctx.stroke();
  ...
  contains(ex, ey) {
    ...
    let cx0 = 0;
    return (
      (ex > cx0) & (ex < cx0 + this.cw) & (ey > cy0) & (ey < cy0 + this.ch)
    );
  }
  pointerDownHandler(ex, ey, en) {
    // log('PD')
...
class ResetButton {
  ...
  draw() {
    ...
    ctx.fillText(
      this.text,
      35 * pixRat,
      this.y0 * Y - 75 * pixRat - this.fontsize
    );
  }
  contains(ex, ey) {
    return (
      (ex > this.cx0) &
      (ex < this.cx0 + this.cw) &
      (ey > this.y0 * Y - 100 * pixRat - this.fontsize) &
      (ey < this.y0 * Y - 100 * pixRat - this.fontsize + this.dh)
    );
  }
  pointerDownHandler(ex, ey, en) {
    // log('PD')
...
let fs = (function () {
  ...
  function drawDebug() {
    ...
    let debugTxt1 = "Air " + car.n.Fair.lon.toFixed(2) + " ";
    ...
    let debugTxt68 =
      ...
      car.wheels[3].load.toFixed(1);
    let debugTxt7 =
      "Res " + car.n.Fres.lon.toFixed(2) + " " + car.n.Fres.lat.toFixed(2);
    let debugTxt8 = "u " + toFixed(car.ulon, 2);
    ...
    let x0 = 100;
    ctx.fillText(debugTxt1, 5, x0 + 10, 500);
    ctx.fillText(debugTxt15, 5, x0 + 20, 500);
    ctx.fillText(debugTxt2, 5, x0 + 30, 500);
    ctx.fillText(debugTxt3, 5, x0 + 40, 500);
    ctx.fillText(debugTxt4, 5, x0 + 50, 500);
    ctx.fillText(debugTxt5, 5, x0 + 60, 500);
    ctx.fillText(debugTxt55, 5, x0 + 70, 500);
    ctx.fillText(debugTxt56, 5, x0 + 80, 500);
    ctx.fillText(debugTxt6, 5, x0 + 90, 500);
    ctx.fillText(debugTxt65, 5, x0 + 100, 500);
    ctx.fillText(debugTxt68, 5, x0 + 110, 500);
    ctx.fillText(debugTxt7, 5, x0 + 120, 500);
    ctx.fillText(debugTxt8, 5, x0 + 130, 500);
  ...
  function drawHUD() {
    ...
    ctx.rect(
      ...
      Y - hudY,
      barWidth,
      (-car.torqueWheelHUD.torque / car.maxTorqueHUD) * barHeight
    );
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.rect(
      hudX + barWidth + barWidthSpace,
      Y - hudY,
      barWidth,
      (-car.brakeWheelHUD.brake / car.maxBrakeHUD) * barHeight
    );
    ctx.fill();
  ...
  function formatDurationTenth(duration) {
    if (duration == 0) {
      ...
      // return (duration < 0 ? '-' : '') + h + ':' + pad(m.toString(), 2) + ':' + pad(s.toString(), 2) +'.' + pad(ms.toString(),3);
      return (
        pad(m.toString(), 1) + ":" + pad(s.toString(), 2) + "." + ts.toString()
      );
    }
  }
  ...
  function drawSpeedo() {
    ...
    ctx.lineTo(
      x0 + rad - 0.9 * rad * Math.cos(-th),
      y0 - rad + 0.9 * rad * Math.sin(-th)
    );
    ctx.stroke();
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
...
function anim() {
  ...
  if (viewMode.vState == 1) {
    // overview
    zoom =
      Math.min(X / track.Xi, (Y - (Y / 3) * isTouch) / track.Yi) /
      track.trackScl;
  }
  if (viewMode.vState == 2) {
    // fixed zoom tracking
...
function urlArgHandler() {
  ...
  dev =
    urlParams.has("tt") ||
    urlParams.has("trackDev") ||
    urlParams.has("carDev") ||
    urlParams.has("lb") ||
    urlParams.has("ml");
...
class TimeTravel {
  ...
  draw() {
    let x0 = X / 2 - (fontSizeBase * 3 * pixRat) / 2;
    let xL = X / 2 - fontSizeBase * (3 / 2 + 3) * pixRat;
    let xR = X / 2 - fontSizeBase * (3 / 2 - 3) * pixRat;
    let y0 = this.yPos - 0.5 * fontSizeBase * pixRat;
    ...
    // ctx.beginPath();
    // ctx.strokeStyle = "white";
    // ctx.rect(xL, y0, this.w, this.h);
    // ctx.stroke();
    // ctx.beginPath();
    // ctx.strokeStyle = "white";
    // ctx.rect(xR, y0, this.w, this.h);
    // ctx.stroke();
    ...
    ctx.fillText("<", xL + this.w / 2, this.yPos);
    ctx.beginPath();
    ctx.fillStyle = "white";
    if (timeTravel.ttDays >= 0) {
      ctx.fillStyle = "darkGrey";
    }
    ctx.fillText(">", xR + this.w / 2, this.yPos);
  }
  contains0(ex, ey) {
    let x0 = X / 2 - (fontSizeBase * 3 * pixRat) / 2;
    // let xL = X / 2 - fontSizeBase * (3 / 2 + 3) * pixRat;
  ...
  containsL(ex, ey) {
    ...
    // let xR = X / 2 - fontSizeBase * (3 / 2 - 3) * pixRat;
    let y0 = this.yPos - 0.5 * fontSizeBase * pixRat;
    return (ex > xL) & (ex < xL + this.w) & (ey > y0) & (ey < y0 + this.h);
  }
  containsR(ex, ey) {
    // let x0 = X / 2 - (fontSizeBase * 3 * pixRat) / 2;
    // let xL = X / 2 - fontSizeBase * (3 / 2 + 3) * pixRat;
    let xR = X / 2 - fontSizeBase * (3 / 2 - 3) * pixRat;
    let y0 = this.yPos - 0.5 * fontSizeBase * pixRat;
    return (ex > xR) & (ex < xR + this.w) & (ey > y0) & (ey < y0 + this.h);
  }
  pointerDownHandler(ex, ey, en) {
    // log('PD')
...
class DocPanel {
  constructor() {
    ...
    document
      .getElementById("closeDocs")
      .addEventListener("click", this.hideDocs, { passive: true });
    if (this.showDocs) {
      this.unhideDocs();
    }
...
if (
  (dev ||
    ...
    location.hostname === "") & !serverOveride
) {
